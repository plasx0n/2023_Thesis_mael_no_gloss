...
Il implante l'ISA de base RV64I, supporte nativement les extensions M/C/F et A de la spécification RISC-V et intègre un mécanisme de prédiction de branchement. En interne, en fonction de la configuration du cœur, ce dernier peut opérer sur des données codées sur 32 bits ou 64 bits.
...
En effet, il faut compter environ une heure pour réaliser une synthèse et le placement routage du SoC contenant le cœur CVA6. Avec 45 configurations différentes, une quantitée non-négligeable de temps est nécessaire. 
%
%
%
% 
...
Il est à noter que les instructions ajoutées dans le cœur CVA6 n'ont pas d'impact sur la fréquence maximale de fonctionnement, probablement liée à la fréquence de fonctionnement limitée à 50 MHz. 
...
Il est à noter que toutes ces instructions sont implantées sans conséquences sur la fréquence de fonctionnement du cœur, pour les raisons mentionées précédemment.}
...
En effet, les accès non réguliers à la mémoire lors des calculs des CNs induisent des pénalités qui restent très prégnantes et ces dernières compromettent une partie des gains liés à nos extensions.
