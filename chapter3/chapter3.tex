...
Les coeurs \IBEX,  \SCR\space  et \RISCY\space sont simulés via l'utilisation de \textit{Verilator} et \textit{iverilog} pour le (\PicoRV).}
\label{flot}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{table}[]
    ...
    \begin{tabular}{l|l}
         ...
         Fonction 7             & [31:25]  \\
    \end{tabular}
    \caption{Format des instructions de type R issues de la spécification RISC-V.}
\end{table}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\end{lstlisting}
\end{figure}
Une analyse complète de l'algorithme \textit{SC} a permis d'identifier jusqu'à 6 motifs pouvant bénéficier d’instructions dédiées, comme cela est résumé dans le tableau \ref{polar_instr}. Il est à noter que la même procédure a été appliquée sur l'algorithme \textit{Fast-SC} qui est une extension de l'algorithme \textit{SC}. Cela a permis d'identifier 5 instructions supplémentaires pertinentes à implanter en matériel, comme mentionné dans le tableau \ref{polar_instr}. Nous ne présentons pas d’effort permettant d’identifier un compromis entre materiel et accélération. Toutefois, l’on peut penser mettre en place des fonctions de coût afin de définir ce dernier.
%%%%%%%%%%%%%%%%
\begin{table}[!tb]
    \centering
    \footnotesize
...
Toutefois, il est possible de les transposer sur 32 bits au format entier scalaire, soit 4 éléments de 8 bits.
\begin{table}[!tb]
    \footnotesize
    \centering
...
Un sous-ensemble des résultats de mesure concernant les temps d'exécution et les débits des différents décodeurs est présenté dans le tableau \ref{Cycles_cores}. L'ensemble des paramètres des codes utilisés sont reportés dans les annexes.
...
Les débits sont calculés en fonction de la fréquence maximale obtenue après placement et routage par l’outil Vivado sur cible FPGA avec la formule suivante : $Nb\_bits/(Latence(CH)∗(1/Fmax)$.
...
Ce dernier résultat s'explique par l'utilisation d'un utilitaire intégré dans le flot de synthèse utilisant Vivado, nommé FuseSoc. Ne maîtrisant pas pleinement ce dernier, nous supposons qu’il utilise Vivado d’une manière différente en fonction des paramètres passés.
