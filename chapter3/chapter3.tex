...
\label{chapter:3}
\chaptermark{Méthodologie de conception et d'implantation d'extensions}
%
% 
%
%
...
\textcolor{blue}{Les coeurs \IBEX,  \SCR\space  et \RISCY\space sont simulés via l'utilisation de \textit{Verilator} et \textit{iverilog} pour le (\PicoRV).}}
\label{flot}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

L'ensemble de ces instructions ont, par la suite, été décrites au niveau RTL en \textit{Verilog} ou \textit{System-Verilog} (\ding{185}). Ensuite, leur ajout dans l'ISA des cœurs RISC-V (\ding{188}) \cite{ArchiRISC:V} a été effectué.
...
\begin{table}[]
    ...
    \begin{tabular}{l|l}
         ...
         Fonction 7             & [31:25]  \\
    \end{tabular}
    \caption{\textcolor{blue}{Format des instructions de type R issues de la spécification RISC-V.}}
\end{table}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Dans le cadre de ces travaux de thèse, l'algorithme considéré est le \textit{\textbf{M}in-\textbf{S}um/} (\textit{MS}) offrant le meilleur compromis entre complexité d'intégration \textit{vs} performances de décodage. De manière analogue aux architectures matérielles \cite{LDPC:APPROX:3,LDPC:APPROX:4} et aux implantations logicielles sur cibles multicœurs \cite{LDPC:SOFT4}, la formulation par couche horizontale des calculs de parité a été sélectionnée afin d'accélérer la vitesse de convergence du décodeur. \textcolor{blue}{L'ordonnancement par inondation (flooding scheduling) n'a pas été considéré dans ces travaux pour des questions de performances, car celui-ci est plus lent pour converger vers la solution. Toutefois, les opérations dans les nœuds de parité sont similaires, par exemple les calculs des minimaux, il peuvent ainsi tirer partie des instructions spécialisées mises en place dans le cadre de cette thèse.} 
...
\textcolor{blue}{Il est à noter que le listing \ref{ldpc_code_origin} est un extrait de l'algorithme complet, en effet celui-ci ne présente que la première partie de ce dernier. Les accès mémoires sont effectués par lecture d'un tableau d'index calculé à partir de la matrice de parité \textit{H} utilisée. Les degrés des nœuds sont également pré-calculés après analyse de la matrice de parité. Le code LDPC employé est un code LDPC(34,20) avec une matrice \textit{H} BG1 pour la 5G. L'algorithme effectue 10 itérations pour 1 trame à décoder. Les données internes sont fixées sur 8 bit signés.}
...
\begin{lstlisting}[
    label={ldpcnb_code},
    caption={Extraits de fonctions issues de l'algorithme de décodage \textcolor{blue}{LDPC-NB avec ordonnancement par couches horizontales, sans instructions spécifiques.}},
    frame=single,
    backgroundcolor=\color{white},  
    commentstyle=\color{blue}\bfseries,
    keywordstyle=\color{magenta},
...
Cette analyse s'est poursuivie sur l'ensemble du processus de décodage. Le tableau \ref{ldpcnb_instr} regroupe l'ensemble des instructions retenues pour accélérer le décodage des LDPC-NB. \textcolor{blue}{Le nombre réduit d'instructions s'explique ici par la faible diversité des opérations arithmétiques utilisées dans cet algorithme de décodage, et par la prépondérance des accès mémoire mis en œuvre, par exemple pour réaliser les opérations de multiplication et de division dans $\mathbb{GF}$. Ce faible nombre d'instructions, en regard de la quantité d’accès mémoires à réaliser, est une contrainte de l'approche proposée. En effet, les instructions que nous proposons dans ce document n’ont pas pour objectif d'optimiser les accès en mémoire.}
...
\end{lstlisting}
\end{figure}
\textcolor{blue}{ Une analyse complète de l'algorithme \textit{SC} a permis d'identifier jusqu'à 6 motifs pouvant bénéficier d’instructions dédiées, comme cela est résumé dans le tableau \ref{polar_instr}. Il est à noter que la même procédure a été appliquée sur l'algorithme \textit{Fast-SC} qui est une extension de l'algorithme \textit{SC}. Cela a permis d'identifier 5 instructions supplémentaires pertinentes à implanter en matériel, comme mentionné dans le tableau \ref{polar_instr}. Nous ne présentons pas d’effort permettant d’identifier un compromis entre materiel et accélération. Toutefois, l’on peut penser mettre en place des fonctions de coût afin de définir ce dernier.}
%%%%%%%%%%%%%%%%
\begin{table}[!tb]
    \centering
    \footnotesize
...
\textcolor{blue}{Il est intéressant de noter qu'il existe un sous-ensemble d'instructions commun à tout ou partie des codes CCE. Ainsi, afin d'accélérer l'ensemble des décodeurs, seules 17 instructions sont nécessaires.
Il est à noter que ces instructions, décrites en langage de description matériel, pourraient être décrites en langage pour la synthèse logique de haut niveau (HLS) afin que cela soit possible, il serait nécessaire de modifier l’outil de HLS de manière identique ) la chaîne de compilation, ou bien de définir des «pragma» pour contraindre la synthèse.}.
...
Dans certains cœurs des mécanismes plus complexes, tel que la capacité d’exécution dans le désordre (\textit{i.e. Out-of-Order}) et/ou l'utilisation de front-end super-scalaire ont été intégrés.  \textcolor{blue}{Toutefois, chaque cœur est potentiellement extensible et modifiable pour permettre l'ajout d'instructions dédiées. Cependant, leurs complexités architecturales et la qualité de leurs descriptions sources peuvent parfois rendre complexe, voir compromettre, ce type d'opération.}
Dans le cadre de ces travaux une vaste étude préliminaire à été effectué. Cette étude a consisté en des mesures de performances de multiples cœurs open-sources de la littérature. Ces comparatifs ont pris en compte la vitesse de prototypage, la complexité de mise en œuvre des benchmarks, la facilité de prise en main des outils proposés, ainsi que la qualité des sources HDL. Au terme de celle-ci, quatre cœurs RISC-V ont été sélectionnés : \PicoRV \cite{wolf_cliffordwolfpicorv32_2021}, \RISCY \cite{cite_riscy}, \IBEX \cite{noauthor_lowriscibex_2021,schiavone_slow_2017} et \SCR \cite{noauthor_syntacorescr1_2021}.
...
\begin{table}
    ...
    \begin{tabular}{lcclc}
    \toprule
                    & Pipelines  & CoreMark/MHz &  Complexité (LUTs | FFs) & Sources     \\
    \hline
    \PicoRV   &     1  &     0.4   &    978  |   567       & Verilog     \\
    \IBEX     &     2  &     2.47  &  2 446  |   883      & Sys. Verilog\\
    \SCR     &     3  &     2.43   &  3 986  | 2 339      & Sys. Verilog\\
    \RISCY    &     4  &     3.19  &  10 302 | 3 033     & Sys. Verilog\\
    \bottomrule
    \end{tabular}
    \caption{\textcolor{blue}{Comparatifs de cœurs de processeurs RISC-V sélectionnés}}
    \label{brief_cores}
\end{table}
...
\textcolor{blue}{Pour mener à bien ces expérimentations, les programmes décrivant le comportement des décodeurs ont été écrits en langage C, en ciblant une exécution directement sur le matériel sans système d’exploitation (en baremetal).} Afin de compiler l'ensemble de ces codes et de produire les binaires exécutés par les cœurs RISC-V, GCC 12.2 a été utilisé. Le compilateur est configuré afin de générer un exécutable ne comportant que des instructions RV32I et avec le flag d'optimisation -O3. 
...
Un sous-ensemble des résultats de mesure concernant les temps d'exécution et les débits des différents décodeurs est présenté dans le tableau \ref{Cycles_cores}. \textcolor{blue}{L'ensemble des paramètres des codes utilisés sont reportés dans les annexes.}
...
Il est important de noter que le gain en pourcentage est la métrique la plus cohérente au vu des différences d'architectures des cœurs de tests. 
\textcolor{blue}{Les débits sont calculés en fonction de la fréquence maximale obtenue après placement et routage par l’outil Vivado sur cible FPGA avec la formule suivante : Nb\_bits/(Latence(CH)∗(1/Fmax). } 
...
\textcolor{blue}{Si l'on considère les autres extensions pour lesquelles le nombre d'instructions ajoutées est moins important, les surcoûts sont plus faibles. Cependant, l'analyse fine de ces différents résultats, et le positionnement du coût des extensions les unes par rapport aux autres n'est pas triviale, du fait des choix opérés par Vivado lors de la synthèse logique et du placement/routage que nous ne maîtrisons pas.}
...
\textcolor{blue}{Ce dernier résultat s'explique par l'utilisation d'un utilitaire intégré dans le flot de synthèse utilisant Vivado, nommé FuseSoc. Ne maîtrisant pas pleinement ce dernier, nous supposons qu’il utilise Vivado d’une manière différente en fonction des paramètres passés. }
Pour le \SCR, la fréquence varie de $-1\%$ à $+1\%$.
